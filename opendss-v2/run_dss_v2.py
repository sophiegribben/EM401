...
def run_dss_v2(networkNum, feederNum, pvPer, pRating, latitude, longitude, loadID, leap, timeLoad):
    ...
    #note to sophie - replace the following with your LCL load file name (columns are meters)
    #then replace the date range with the start and end dates you have
    #if timeLoad == 1:
    loadAllClean = pd.read_csv("lcl_clean_noheadings.csv", header=None).to_numpy()
    t = pd.date_range(start='01/01/2018', end='31/12/2018 23:30:00', freq='0.5H')
    t_target = pd.date_range(start='7/26/2015', end='8/30/2015 23:30:00', freq='H')#not used
    # elif timeLoad == 2: 
    ...
    phase1 = pd.DataFrame(phase1, columns=['max voltage', 'P(kW)', 'Q(kVAR)', 'v_angle'], index=t)
    phase2 = pd.DataFrame(phase2, columns=['max voltage', 'P(kW)', 'Q(kVAR)', 'v_angle'], index=t)
    phase3 = pd.DataFrame(phase2, columns=['max voltage', 'P(kW)', 'Q(kVAR)', 'v_angle'], index=t)
    ...
    os.remove('MTR_ss1.csv')
     # %%   
    return phase1, phase2, phase3, feederMaxVolt, feederPower, feederReactive, nodeV, t
